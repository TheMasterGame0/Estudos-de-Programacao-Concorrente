...
void* reader(void *arg) {
    ...
    pthread_mutex_lock(&turno); /* Garante a chance do escritor escrever */
    pthread_mutex_lock(&mutex); /* Evita dois leitores tentarem bloquear o bd ao mesmo tempo*/   
    rc = rc + 1;            
    if (rc == 1) {          
...
void* writer(void *arg) {
    ...
    pthread_mutex_lock(&turno);   /* Garante que não tenha leitores sempre*/
      pthread_mutex_lock(&db);    /* obtém acesso exclusivo ao BD*/
    pthread_mutex_unlock(&turno); /* Pode ser liberado pois os BD irá segurar os leitores agora*/
